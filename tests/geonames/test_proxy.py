...
class TestGeoNamesProxy:
    ...
    async def test_get_details_with_extra_params(self, mock_client_class):
        ...
        async with proxy:
            await proxy.get_details(123, lang="sv", style="MEDIUM", extra_params={"include_bbox": True})
    ...
    async def test_geonames_api_error_handling(self, mock_client_class):
        ...
        error_response = {"status": {"message": "user account not enabled to use the free webservice", "value": 10}}
    ...
    def test_ensure_ok_static_method_with_error(self):
        ...
        mock_response = MagicMock()
        mock_response.raise_for_status.return_value = None
        mock_response.json.return_value = {"status": {"message": "rate limit exceeded", "value": 19}}
...
class TestGeoNamesProxyIntegration:
    ...
    async def test_complete_search_workflow(self, mock_client_class):
        ...
        search_response = {"geonames": [{"geonameId": 2666199, "name": "Umeå", "lat": "63.82842", "lng": "20.25972"}]}
