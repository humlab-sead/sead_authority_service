...
import psycopg
import pytest
from strategies.interface import ReconciliationStrategy, StrategySpecification
from src.strategies.query import QueryProxy
from src.strategies.dimension import DimensionReconciliationStrategy, DimensionQueryProxy
from src.strategies.method import MethodReconciliationStrategy, MethodQueryProxy
...
from src.strategies.data_type import DataTypeReconciliationStrategy, DataTypeQueryProxy
from src.strategies.feature_type import SPECIFICATION as FEATURE_TYPE_SPECIFICATION

from src.strategies.feature_type import FeatureTypeReconciliationStrategy, FeatureTypeQueryProxy
...
#     @pytest.mark.asyncio
#     @with_test_config
#     async def test_reconciliation_strategy(self, specification: StrategySpecification, strategy_class: Type[ReconciliationStrategy], query_proxy_class: Type[QueryProxy], test_provider: MockConfigProvider) -> None:
...
#         mock_rows = [{id_field: 1, "label": f"Test {key.capitalize()} 1", "name_sim": 0.9}, {id_field: 2, "label": f"Test {key.capitalize()} 2", "name_sim": 0.8}]
