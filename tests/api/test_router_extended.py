...
class TestSuggestEntityEndpoint:
    ...
    async def test_suggest_entity_success(
        self, mock_suggest: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    @with_test_config
    @patch("src.api.router.suggest_entities")
    async def test_suggest_entity_with_type_filter(
        self, mock_suggest: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    @with_test_config
    @patch("src.api.router.suggest_entities")
    async def test_suggest_entity_empty_prefix(
        self, mock_suggest: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    async def test_suggest_entity_error(
        self, mock_suggest: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    @with_test_config
    @patch("src.api.router.suggest_entities")
    async def test_suggest_entity_multiple_results(
        self, mock_suggest: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
...
class TestSuggestTypeEndpoint:
    ...
    async def test_suggest_type_success(
        self, mock_suggest: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    @with_test_config
    @patch("src.api.router.suggest_types")
    async def test_suggest_type_with_prefix(
        self, mock_suggest: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    async def test_suggest_type_error(
        self, mock_suggest: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    @with_test_config
    @patch("src.api.router.suggest_types")
    async def test_suggest_type_empty_result(
        self, mock_suggest: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
...
class TestSuggestPropertyEndpoint:
    ...
    async def test_suggest_property_success(
        self, mock_suggest: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    @with_test_config
    @patch("src.api.router.suggest_properties_api")
    async def test_suggest_property_with_prefix(
        self, mock_suggest: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    @with_test_config
    @patch("src.api.router.suggest_properties_api")
    async def test_suggest_property_with_type(
        self, mock_suggest: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    @with_test_config
    @patch("src.api.router.suggest_properties_api")
    async def test_suggest_property_with_both_filters(
        self, mock_suggest: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    async def test_suggest_property_error(
        self, mock_suggest: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
...
class TestFlyoutEntityEndpoint:
    ...
    async def test_flyout_entity_get_success(
        self, mock_render: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    @with_test_config
    @patch("src.api.router.render_flyout_preview")
    async def test_flyout_entity_post_success(
        self, mock_render: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    @with_test_config
    @patch("src.api.router.render_flyout_preview")
    async def test_flyout_entity_missing_id(
        self, mock_render: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    @with_test_config
    @patch("src.api.router.render_flyout_preview")
    async def test_flyout_entity_empty_id(
        self, mock_render: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    async def test_flyout_entity_invalid_id(
        self, mock_render: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    async def test_flyout_entity_render_error(
        self, mock_render: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
...
class TestReconcileEndpointExtended:
    ...
    async def test_reconcile_json_content_type(
        self, mock_reconcile: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
        """Test reconcile with JSON content type."""
        mock_reconcile.return_value = {
            "q0": {"result": [{"id": "1", "name": "Test", "score": 95.0, "match": True, "type": []}]}
        }

        queries = {"q0": {"query": "Test", "type": "site"}}
        response = client.post(
            "/reconcile", json={"queries": queries}, headers={"Content-Type": "application/json"}
        )
    ...
    @with_test_config
    @patch("src.api.router.reconcile_queries")
    async def test_reconcile_double_encoded_queries(
        self, mock_reconcile: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    @with_test_config
    @patch("src.api.router.reconcile_queries")
    async def test_reconcile_adds_default_type(
        self, mock_reconcile: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    @with_test_config
    @patch("src.api.router.reconcile_queries")
    async def test_reconcile_multiple_queries(
        self, mock_reconcile: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    @with_test_config
    @patch("src.api.router.reconcile_queries")
    async def test_reconcile_with_properties(
        self, mock_reconcile: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    @with_test_config
    @patch("src.api.router.reconcile_queries")
    async def test_reconcile_with_limit(
        self, mock_reconcile: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    @with_test_config
    @patch("src.api.router.reconcile_queries")
    async def test_reconcile_with_type_strict(
        self, mock_reconcile: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
...
class TestContentTypeHandling:
    ...
    def test_reconcile_missing_queries_field(self, client: TestClient, test_provider: MockConfigProvider):
        ...
        response = client.post(
            "/reconcile", data={"other": "data"}, headers={"Content-Type": "application/x-www-form-urlencoded"}
        )
...
class TestResponseModels:
    ...
    @with_test_config
    @patch("src.api.router.suggest_entities")
    async def test_suggest_entity_response_model(
        self, mock_suggest: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    @with_test_config
    @patch("src.api.router.suggest_types")
    async def test_suggest_type_response_model(
        self, mock_suggest: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
    ...
    @with_test_config
    @patch("src.api.router.suggest_properties_api")
    async def test_suggest_property_response_model(
        self, mock_suggest: AsyncMock, client: TestClient, test_provider: MockConfigProvider
    ):
