 ...
 ** What      Stores 768-dimensional embeddings for semantic search over taxonomic genus
...
as $$
  ...
  from authority.taxa_tree_genus as g
  cross join params pq
  where g.norm_label % pq.q
...
as $$
  select g.genus_id, g.label, 1.0 - (g.emb <=> qemb) as sem_sim
  from authority.taxa_tree_genus as g
...
as $$
  ...
  trgm as (
    select
      ...
      g.label,
      greatest(
        case when g.norm_label = pq.q then 1.0
            else similarity(g.norm_label, pq.q)
        end,
        0.0001
      ) as trgm_sim
    from authority.taxa_tree_genus as g
