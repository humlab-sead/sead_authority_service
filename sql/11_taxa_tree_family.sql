...
DROP VIEW IF EXISTS authority.taxa_tree_families;
CREATE OR REPLACE VIEW authority.taxa_tree_families AS
  SELECT  
    f.family_id,
...
DROP FUNCTION IF EXISTS authority.fuzzy_taxa_tree_families(TEXT, INTEGER);
CREATE OR REPLACE FUNCTION authority.fuzzy_taxa_tree_families(
  p_text  TEXT,
...
AS $$
  ...
  FROM authority.taxa_tree_families AS f
...
/***************************************************************************************************
 ** Procedure  authority.semantic_taxa_tree_families
 ** What       Semantic search function using pgvector embeddings
 ****************************************************************************************************/
DROP FUNCTION IF EXISTS authority.semantic_taxa_tree_families(VECTOR, INTEGER);

CREATE OR REPLACE FUNCTION authority.semantic_taxa_tree_families(
  qemb    VECTOR,
  p_limit INTEGER DEFAULT 10
)
RETURNS TABLE (
...
/***************************************************************************************************
 ** Procedure  authority.search_taxa_tree_families_hybrid
 ** What       Hybrid search combining trigram and semantic search
 ** Notes      See docs/MCP Server/SEAD Reconciliation via MCP — Architecture Doc (Outline).md
 ****************************************************************************************************/
DROP FUNCTION IF EXISTS authority.search_taxa_tree_families_hybrid(TEXT, VECTOR, INTEGER, INTEGER, INTEGER, DOUBLE PRECISION);

CREATE OR REPLACE FUNCTION authority.search_taxa_tree_families_hybrid(
  p_text  TEXT,
  qemb    VECTOR,
