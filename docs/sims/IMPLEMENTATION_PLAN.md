...
| D3 | Migration tool | **Sqitch** | Consistent with SEAD Change Control System already in use. |
...
| Task | Notes |
|------|-------|
| Select pilot entity types | Start with `site` and `sample_group` — provider-owned, well-understood |
| Seed identity policy for pilot types | Add entries to `config/identity_policy.yml` |
| Create test submission with known identities | Manual or scripted via API |
| Validate Resolve → Bind → Confirm flow end-to-end | Verify binding set lifecycle, tracked identity allocation |
| Validate idempotency | Same submission twice → same bindings, no duplicate tracked identities |
| Validate content hash change detection | Submit → detect insert, resubmit identical → skip, modify → update |
| Document any issues found | Feed back into design docs if needed |

**Acceptance:** Full happy-path flow works for provider-owned entities. Idempotency holds. Change detection works. No regressions in reconciliation endpoints.
