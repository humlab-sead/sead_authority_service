...
async def render_preview(id: str, id_base: str, cursor) -> ValueError | str:
    ...
    strategy = next((s for s in app.state.strategies.values() if s.get_id_path() == entity_path), None)
    ...
    try:
        ...
        if not details:
            raise ValueError(f"Entity with ID {entity_id_str} not found or preview not implemented.", status_code=404)
