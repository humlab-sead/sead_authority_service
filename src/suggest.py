...
async def suggest_types(prefix: str = "") -> dict[str, Any]:
    ...
    all_types = [
        ...
        for type_key in Strategies.items.keys()
    ]
    ...
    if prefix:
        ...
        filtered_types = [
            ...
            if prefix_lower in t["id"].lower() or prefix_lower in t["name"].lower()
        ]
    else:
        filtered_types = all_types
    
    return {
        "result": filtered_types
    }
...
async def suggest_properties(prefix: str = "", entity_type: str = "") -> dict[str, Any]:
    ...
    if prefix:
        prefix_lower = prefix.lower()
        property_suggestions = [
            p for p in property_suggestions
            if prefix_lower in p["id"].lower() or 
               prefix_lower in p["name"].lower() or 
               prefix_lower in p.get("description", "").lower()
        ]
