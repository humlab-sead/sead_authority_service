...
create or replace view authority.bibliographic_reference as  select
    ...
    authority.immutable_unaccent(lower(t.full_reference)) as norm_label,    t.title,    t.doi,    t.bugs_reference,    t.isbn,    t.notes,    t.year,    t.authors,    t.url,    authority.immutable_unaccent(lower(t.bugs_reference)) as norm_bugs_reference,    authority.immutable_unaccent(lower(t.title)) as norm_title,    authority.immutable_unaccent(lower(t.authors)) as norm_authors,    e.emb
  from public.tbl_biblio as t  left join authority.bibliographic_reference_embeddings as e using (biblio_id);
...
as $$
  ...
  , sem as (
    ...
    from authority.bibliographic_reference as e    where e.emb is not null    order by e.emb <=> qemb
    limit k_sem
  )
  , u as (
