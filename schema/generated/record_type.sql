...
create or replace view authority.record_type as  select
    ...
    authority.immutable_unaccent(lower(t.record_type_name)) as norm_label,    t.record_type_description,    e.emb
  from public.tbl_record_types as t  left join authority.record_type_embeddings as e using (record_type_id);
...
as $$
  ...
  , sem as (
    ...
    from authority.record_type as e    where e.emb is not null    order by e.emb <=> qemb
    limit k_sem
  )
  , u as (
