...
create or replace view authority.feature_type as  select
    ...
    authority.immutable_unaccent(lower(t.feature_type_name)) as norm_label,    t.feature_type_description,    e.emb
  from public.tbl_feature_types as t  left join authority.feature_type_embeddings as e using (feature_type_id);
...
as $$
  ...
  , sem as (
    ...
    from authority.feature_type as e    where e.emb is not null    order by e.emb <=> qemb
    limit k_sem
  )
  , u as (
