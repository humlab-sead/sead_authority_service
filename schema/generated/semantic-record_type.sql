 ...
 ** Note   Embeddings side table for Record Type in schema authority
...
create table if not exists authority.record_type_embeddings (
...
create index if not exists record_type_embeddings_ivfflat
  on authority.record_type_embeddings
    using ivfflat (emb vector_cosine_ops)
      with (lists = 100);
/***************************************************************************************************
 ** Procedure  authority.update_record_type_embeddings
 ** What       Updates embeddings in authority.record_type_embeddings table
 ** Usage      SELECT authority.update_record_type_embeddings();        -- Update only missing
 **            SELECT authority.update_record_type_embeddings(true);    -- Force update all
 ** Arguments  p_force_update: If true, regenerate all embeddings; if false (default), only compute missing ones
 ** Returns    Number of rows updated
 ****************************************************************************************************/
 
drop function if exists authority.update_record_type_embeddings(boolean) cascade;

create or replace function authority.update_record_type_embeddings(
...
begin
  raise notice 'Updating record_type embeddings (force_update: %)', p_force_update;
  
  for rec in
    select t.record_type_id,
           t.record_type_name,
           t.record_type_description    from public.tbl_record_types t
    where p_force_update 
       or not exists (
         select 1 from authority.record_type_embeddings e 
         where e.record_type_id = t.record_type_id
       )
  loop
    -- Construct text for embedding (combine label and description if available)
    v_text := rec.record_type_name;    if rec.record_type_description is not null then
      v_text := v_text || ' ' || rec.record_type_description;
    end if;    
    ...
    insert into authority.record_type_embeddings (record_type_id, emb)
    values (rec.record_type_id, v_emb)
    on conflict (record_type_id) do update
  ...
  raise notice 'Completed: % rows updated for record_type', v_rows_updated;
  return v_rows_updated;
end;
$$;

/**********************************************************************************************
**  Record Type - Semantic Search Objects (Vector Embeddings)
**  Generated from template by generate_entity_schema.py
**  
**  Note: This file does NOT modify authority.record_type view.
 ...
 ** Usage      SELECT * FROM authority.semantic_record_type(qemb::vector, 10);
 ** Note       Joins authority.record_type view with authority.record_type_embeddings table
 ****************************************************************************************************/
...
as $$
  ...
  from authority.record_type as v
  inner join authority.record_type_embeddings as e using (record_type_id)
...
as $$
  with params as (
    ...
    from authority.record_type as e
  ...
  , sem as (
    ...
    from authority.record_type as v
    inner join authority.record_type_embeddings as emb using (record_type_id)    where emb.emb is not null    order by emb.emb <=> qemb
    limit k_sem
  )
  , u as (
