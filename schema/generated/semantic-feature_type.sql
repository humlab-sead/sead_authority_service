 ...
 ** Note   Embeddings side table for Feature Type in schema authority
...
create index if not exists feature_type_embeddings_ivfflat
  on authority.feature_type_embeddings
    using ivfflat (emb vector_cosine_ops)
      with (lists = 100);
/***************************************************************************************************
 ** Procedure  authority.update_feature_type_embeddings
 ** What       Updates embeddings in authority.feature_type_embeddings table
 ** Usage      SELECT authority.update_feature_type_embeddings();        -- Update only missing
 **            SELECT authority.update_feature_type_embeddings(true);    -- Force update all
 ** Arguments  p_force_update: If true, regenerate all embeddings; if false (default), only compute missing ones
 ** Returns    Number of rows updated
 ****************************************************************************************************/
 
drop function if exists authority.update_feature_type_embeddings(boolean) cascade;

create or replace function authority.update_feature_type_embeddings(
...
begin
  raise notice 'Updating feature_type embeddings (force_update: %)', p_force_update;
  
  for rec in
    select t.feature_type_id,
           t.feature_type_name,
           t.feature_type_description    from public.tbl_feature_types t
    where p_force_update 
       or not exists (
         select 1 from authority.feature_type_embeddings e 
         where e.feature_type_id = t.feature_type_id
       )
  loop
    -- Construct text for embedding (combine label and description if available)
    v_text := rec.feature_type_name;    if rec.feature_type_description is not null then
      v_text := v_text || ' ' || rec.feature_type_description;
    end if;    
    ...
    insert into authority.feature_type_embeddings (feature_type_id, emb)
    values (rec.feature_type_id, v_emb)
    on conflict (feature_type_id) do update
  ...
  raise notice 'Completed: % rows updated for feature_type', v_rows_updated;
  return v_rows_updated;
end;
$$;

/**********************************************************************************************
**  Feature Type - Semantic Search Objects (Vector Embeddings)
**  Generated from template by generate_entity_schema.py
**  
**  Note: This file does NOT modify authority.feature_type view.
 ...
 ** Usage      SELECT * FROM authority.semantic_feature_type(qemb::vector, 10);
 ** Note       Joins authority.feature_type view with authority.feature_type_embeddings table
 ****************************************************************************************************/
...
as $$
  ...
  from authority.feature_type as v
  inner join authority.feature_type_embeddings as e using (feature_type_id)
...
as $$
  with params as (
    ...
    from authority.feature_type as e
  ...
  , sem as (
    ...
    from authority.feature_type as v
    inner join authority.feature_type_embeddings as emb using (feature_type_id)    where emb.emb is not null    order by emb.emb <=> qemb
    limit k_sem
  )
  , u as (
