...
create or replace view authority.method as  select
    ...
    authority.immutable_unaccent(lower(t.method_name)) as norm_label,    t.description,    t.method_abbrev_or_alt_name,    e.emb
  from public.tbl_methods as t  left join authority.method_embeddings as e using (method_id);
...
as $$
  ...
  , sem as (
    ...
    from authority.method as e    where e.emb is not null    order by e.emb <=> qemb
    limit k_sem
  )
  , u as (
