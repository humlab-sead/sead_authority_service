...
create or replace view authority.taxa_tree_order as  select
    ...
    authority.immutable_unaccent(lower(t.order_name)) as norm_label,    t.record_type_id,    e.emb
  from public.tbl_taxa_tree_orders as t  join public.tbl_record_types rt using (record_type_id)  left join authority.taxa_tree_order_embeddings as e using (order_id);
...
as $$
  ...
  , sem as (
    ...
    from authority.taxa_tree_order as e    where e.emb is not null    order by e.emb <=> qemb
    limit k_sem
  )
  , u as (
