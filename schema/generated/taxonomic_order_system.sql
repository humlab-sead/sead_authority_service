...
create or replace view authority.taxonomic_order_system as  select
    ...
    authority.immutable_unaccent(lower(t.system_name)) as norm_label,    t.system_description,    e.emb
  from public.tbl_taxonomic_order_systems as t  left join authority.taxonomic_order_system_embeddings as e using (taxonomic_order_system_id);
...
as $$
  ...
  , sem as (
    ...
    from authority.taxonomic_order_system as e    where e.emb is not null    order by e.emb <=> qemb
    limit k_sem
  )
  , u as (
