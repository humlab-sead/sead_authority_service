...
create or replace view authority.taxa_tree_family as  select
    ...
    authority.immutable_unaccent(lower(t.family_name)) as norm_label,    t.order_id,    e.emb
  from public.tbl_taxa_tree_families as t  join public.tbl_taxa_tree_orders ord using (order_id)  left join authority.taxa_tree_family_embeddings as e using (family_id);
...
as $$
  ...
  , sem as (
    ...
    from authority.taxa_tree_family as e    where e.emb is not null    order by e.emb <=> qemb
    limit k_sem
  )
  , u as (
