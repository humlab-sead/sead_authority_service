...
create or replace view authority.taxa_tree_author as  select
    t.author_id,
    t.author_name as label,
    authority.immutable_unaccent(lower(t.author_name)) as norm_label,    e.emb
  from public.tbl_taxa_tree_authors as t  left join authority.taxa_tree_author_embeddings as e using (author_id);
...
as $$
  ...
  , sem as (
    ...
    from authority.taxa_tree_author as e    where e.emb is not null    order by e.emb <=> qemb
    limit k_sem
  )
  , u as (
