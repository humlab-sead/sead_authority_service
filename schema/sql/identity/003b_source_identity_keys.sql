...
create table sead_identity.source_identity_keys (
    ...
    key_type text not null check (key_type in ('uuid', 'business_key', 'provider_key', 'authority_key')),
    key_value text not null,

    constraint source_identity_keys_pkey primary key (key_uuid),
    constraint source_identity_keys_type_unique unique (source_identity_uuid, key_type)
);
