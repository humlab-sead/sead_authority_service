...
create index binding_sets_submission_idx on sead_identity.binding_sets (submission_uuid)
    where submission_uuid is not null;

create index binding_sets_change_request_idx on sead_identity.binding_sets (change_request_name)
    where change_request_name is not null;
